...
void Integral(long int segnum, int NT)
{
    double start, end;
    int i;
    double res = 0.0, x_l, x_r, x_mid, F_l, F_r, F_mid;
    ...
    #pragma omp parallel default (none) num_threads(NT) private (i, x_l, x_r, x_mid, F_r, F_l, F_mid) shared (segnum, step_size) reduction(+: res)
    {
        #pragma omp for schedule (static) // распределит нагрузку на нити примерно поровну
        for (i = 0; i < segnum; i++)
        {
